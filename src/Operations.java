...
public class Operations {
	...
	public static String removeStopWords(String text){
		...
		LinkedList<String> wordListReturned = (LinkedList<String>) wordList.clone();
		
		for (String word : wordList) {
			//test
			System.out.println(word);
			if(Arrays.asList(wordList).contains(word) && word != "")
				Arrays.asList(wordListReturned).remove(word);
		}
		
		return wordListReturned.toString();
		...
		for (int i = 0; i < frequancyMatrix.length; i++) {
		    for (int j = 0; j < frequancyMatrix[i].length; j++) {
		    	...
		    	else
		    	{
			    	...
			    	tf[i][j] = (float) (1+Math.log10(frequancyMatrix[i][j])); 		
		    	}
		    }
		}
